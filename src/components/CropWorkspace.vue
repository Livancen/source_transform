...
<template>
  ...
  >
    <div class="px-14px py-12px flex flex-col gap-12px max-h-420px overflow-auto">
      <div class="flex items-center justify-between gap-12px flex-wrap">
        <div>
          <div class="text-13px font-600">自定义裁剪</div>
          <div class="text-11px color-t3 mt-2px">每次处理 1 个文件 · 绝对像素 · 支持图片与视频</div>
        </div>
        <button
      ...
      <div class="flex gap-12px min-h-0 flex-wrap lg:flex-nowrap">
        <!-- 文件选择 -->
        <div class="w-full lg:w-240px shrink-0 border border-border rounded-8px bg-bg0 overflow-hidden flex flex-col max-h-320px">
          <div class="h-32px px-10px flex items-center text-11px font-500 color-t3 border-b border-border bg-bg2">
            选择文件
          </div>
          <div class="flex-1 overflow-y-auto">
            <button
              ...
              :class="selectedPath === f.path ? 'bg-secondary-soft color-secondary' : 'bg-transparent color-t2 hover:bg-bg2'"
              @click="emit('selectFile', f)"
            >
              <span class="font-500">{{ f.name }}</span>
              <span class="ml-6px text-10px color-t3">{{ f.file_type === "video" ? "视频" : "图片" }}</span>
            </button>
            <div v-if="files.length === 0" class="p-16px text-11px color-t3 text-center">输入目录无媒体文件</div>
          </div>
        </div>

        <!-- 预览 -->
        <div class="flex-1 min-w-0 flex flex-col items-start gap-10px">
          <div v-if="isLoading" class="text-12px color-t3 py-40px w-full text-center">加载预览中…</div>
          <div v-else-if="!cropFrameImage" class="text-12px color-t3 py-40px w-full text-center border border-dashed border-border rounded-8px">
            请从左侧选择一个文件
          </div>
          <template v-else>
            <div class="relative inline-block bg-black">
              ...
              <div class="absolute inset-0 w-full h-full pointer-events-none">
                <div class="absolute top-0 left-0 w-full bg-black/50" :style="{ height: cropY * previewScale + 'px' }"></div>
                <div
                  class="absolute bottom-0 left-0 w-full bg-black/50"
                  :style="{ height: (mediaHeight - cropY - cropHeight) * previewScale + 'px' }"
                ></div>
                <div
              ...
              >
                ...
                ></div>
                <div class="absolute bottom-5px left-50% -translate-x-50% bg-black/70 color-white px-8px py-2px rounded-3px text-12px whitespace-nowrap">
            ...
            <div class="flex gap-14px flex-wrap items-center">
              ...
              <span class="text-12px color-t3">原始: {{ mediaWidth }} × {{ mediaHeight }}</span>
              <button class="tb-btn" type="button" @click="emit('clearFile')">清除选择</button>
            </div>
          </template>
        </div>
      </div>
